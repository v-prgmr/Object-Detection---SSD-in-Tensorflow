...
int main(int argc, const char * argv[]) {
	...
	//If you want to use your webcam, uncomment this code block below and remove the manual reference to single image found on line 22
	/*cv::VideoCapture cap(0); // open the default camera
	if (!cap.isOpened())  // check if we succeeded
		return -1;
		*/ 
	//uncomment the below for loop for reading from webcam
	/*for (;;)
	{
		cv::Mat frame;
		cap >> frame;
		cv::imshow("Input", frame);
		cv::Mat imagePath = frame;
		cv::Mat image;
		image = imagePath;
		*/
		
		// = cv::imread(imagePath, CV_LOAD_IMAGE_COLOR); // Read the file
